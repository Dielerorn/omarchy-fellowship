...
Item {
  ...
  function launchScreensaver() {
    ...
    // Upstream launches omarchy-launch-screensaver, which runs ttfx with
    // --random-effect: fireworks, thunderstorm, VHS glitch, each cycle a
    // different palette. fellowship-launch-screensaver is a copy of that
    // script whose only change is spawning fellowship-screensaver, which pins
    // one quiet effect in the theme's golds. Falls back to the stock launcher
    // if the copy is ever missing, so idle never silently stops working.
    // Pick the launcher by existence, not by exit code: both scripts exit
    // non-zero for benign reasons (screensaver toggled off, one already
    // running), and an `||` chain would answer those by starting the stock
    // flashy one instead.
    runProcess(screensaverProcess, "screensaver",
      "[[ $(omarchy-shell lock isLocked 2>/dev/null) == \"true\" ]] || "
      + "{ s=$HOME/.config/omarchy/bin/fellowship-launch-screensaver; "
      + "[[ -x $s ]] || s=omarchy-launch-screensaver; \"$s\"; }")
  }
