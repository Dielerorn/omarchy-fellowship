...
Item {
  ...
  // Upstream sleeps the displays five seconds after the last input on the
  // lock screen. That is off here, for two reasons:
  //
  //   - Only real input re-arms it, so it fires while you are still sitting
  //     at the machine -- lock the screen, stop moving, and the monitors go
  //     dark on you five seconds later.
  //   - Waking from it is unreliable on this box. Hyprland flips dpmsStatus
  //     back to true the instant it sees input, so omarchy-brightness-display
  //     skips its DPMS enable, and no real modeset ever happens. HDMI-A-1
  //     (MAG27CQ, 1440p120 over HDMI) takes the sleep and does not come back
  //     without a forced re-modeset.
  //
  // The timer defaults to running: false and nothing else starts it, so
  // holding it stopped here disables blanking outright and leaves runBlank()
  // unreachable -- kept intact below so restoring the behaviour is a one-line
  // revert of this function.
